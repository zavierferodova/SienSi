...
const setSessionDocumentCookie = (cookie: string) => {
    const encryptedCookie = AES.encrypt(cookie, process.env["NEXT_PUBLIC_COOKIE_SECRET"]!.trim()).toString();
...
const getSessionData = (): AuthModel | null => {
    try {
        ...
        if (cookie) {
            const decryptedCookie = AES.decrypt(cookie, process.env["NEXT_PUBLIC_COOKIE_SECRET"]?.trim() ?? "").toString(CryptoJS.enc.Utf8);
...
const getSessionDataFromRequest = (request: NextRequest): AuthModel | null => {
    try {
        ...
        if (cookie) {
            const decryptedCookie = AES.decrypt(cookie.value, process.env["NEXT_PUBLIC_COOKIE_SECRET"]?.trim() ?? "").toString(CryptoJS.enc.Utf8);
            return JSON.parse(decryptedCookie) as AuthModel;
        }
...
const checkClientSideSession = () => {
    try {
        ...
        if (session) {
            ...
            const { accessToken, refreshToken, user } = JSON.parse(stringifySession) as AuthModel;
...
const checkServerSideSession = async (request: NextRequest): Promise<boolean> => {
    try {
...
const fetchWithSession = async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
